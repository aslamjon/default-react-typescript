...
const CustomIcon = ({ isMulti, selected, isClearAll, clearAll, clickHeader, maxShowSelected }) => {
  return isClearAll && selected.length ? (
    <div className={"select__header__clearAll"} onClick={clearAll}>
      Clear All
    </div>
  ) : isMulti && !isEmpty(selected) && selected.length > maxShowSelected ? (
    <div className="select__header__selectedNumber">+{selected.length - maxShowSelected}</div>
  ) : (
